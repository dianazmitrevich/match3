...
export class Board {
    constructor() {
        ...
        this.create();
        this.ajustPosition(); // центрируем доску
    }
    ...
    createTiles() {
        // проходим по каждому полю и создаем на нем плитку
        this.fields.forEach((field) => this.createTile(field));
    }

    createTile(field) {
        const tile = TileFactory.generate(); // генерирация плитки
        field.setTile(tile);
        this.container.addChild(tile.sprite); // добавляем спрайт плитки в контейнер доски

        tile.sprite.interactive = true;
        tile.sprite.on("pointerdown", () => {
            // при нажатии на плитку вызывается событие tile-touch-start
